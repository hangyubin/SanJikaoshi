...
默认情况下，文件上传目录会映射到项目根目录下的 `uploads` 文件夹。如果需要自定义映射路径，可以修改 `docker-compose.yml` 文件中的 `volumes` 配置：
...
# 修改后（示例：映射到D盘的exam-uploads文件夹）
volumes:
  # 映射文件上传目录，可自定义宿主机路径
  - D:/exam-uploads:/upload
```

同时，您可以通过修改环境变量 `SMART_EXAM_UPLOAD_DIR` 来改变容器内的目录，但建议保持默认的 `/upload` 目录不变，只修改宿主机的映射路径即可。
